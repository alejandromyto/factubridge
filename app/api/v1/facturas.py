...
from app.schemas import FacturaInput, FacturaResponse, ErrorResponse
from app.models import RegistroFacturacion, ObligadoTributario
...
async def crear_factura(
    ...
    try:
        ...
        stmt = select(ObligadoTributario).where(
            ObligadoTributario.nif == nif,
            ObligadoTributario.activo == True
        )
        ...
        stmt_anterior = select(RegistroFacturacion).where(
            RegistroFacturacion.nif_emisor == nif,
            RegistroFacturacion.estado.in_(["correcto", "aceptado_con_errores"])
        ).order_by(RegistroFacturacion.created_at.desc()).limit(1)
        ...
        base_url = "https://prewww2.aeat.es/wlpl/TIKE-CONT/ValidarQR" if settings.env != "production" \
        ...
        logger.info(f"Factura creada: {registro.id} - {factura_input.serie}{factura_input.numero}")
