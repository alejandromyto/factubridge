from sqlalchemy import Column, String, DateTime, Text, Boolean, Integer, ForeignKey, Index, text, Numeric
...
class APIKey(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    key_hash = Column(String(64), unique=True, nullable=False, index=True)  # SHA256 del key
    ...
    __table_args__ = (
        Index("idx_api_keys_nif_activa", "nif", "activa"),
    )
...
class RegistroFacturacion(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    nif_emisor = Column(String(20), ForeignKey("obligados_tributarios.nif"), nullable=False, index=True)
    ...
    __table_args__ = (
        ...
        Index("idx_registros_factura_unica", "nif_emisor", "serie", "numero", "fecha_expedicion"),
...
class Webhook(Base):
    ...
    eventos = Column(JSONB, nullable=False)  # ["registro.correcto", "registro.incorrecto"]
    activo = Column(Boolean, default=True, nullable=False)
    secret = Column(String(64))  # Para firmar payload HMAC
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    
    __table_args__ = (
        Index("idx_webhooks_nif_activo", "nif", "activo"),
    )
...
class EventoWebhook(Base):
    ...
    registro_id = Column(UUID(as_uuid=True), ForeignKey("registros_facturacion.id"), nullable=False)
